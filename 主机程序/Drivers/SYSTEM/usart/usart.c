...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
		...
		else if (huart->Instance == UART4)
    {
        ...
        if (uart4_rx_finish_flag == 0)
        {
            if (uart4_rx_byte == '\n')
            {
                uart4_rx_finish_flag = 1;
                uart4_rx_buf[uart4_rx_index] = '\0';
            }
            else
            {
                if (uart4_rx_index < 128 - 1) // UART4_RX_BUF_SIZE is 128
                {
                    uart4_rx_buf[uart4_rx_index++] = uart4_rx_byte;
                }
                else
                {
                    uart4_rx_index = 0;
                    memset(uart4_rx_buf, 0, 128);
                }
            }
        }
